...
begin
  ...
  for i := 1 to 9 do begin
    TImage( FindComponent('Image'+inttostr(i))).Picture.LoadFromFile('i.png.png');
...
begin
  ...
  if (player = 1) then begin
    (Sender as TImage).Picture.LoadFromFile('i_x.png.png');
  ...
  if (player = 2) then begin
    (Sender as TImage).Picture.LoadFromFile('i_o.png.png');
    (Sender as TImage).Enabled:=false;
    a[i,j] := player;
    CountClick := CountClick +1;
    isFinish ( Sender );
